 ...
 Get-ChildItem -Path $PSScriptRoot -Filter "*.xlsx" | ForEach-Object{
    ...
    Write-Output "">key.txt
     for ($i = 1; $i -le $countUsed; $i++) {
         ...
         #$SEL = Select-String -Path key.txt -Pattern $uid.guid

         if ( $tc -ne $null)
         {
            # echo "Contains NOTHING String"
            Write-Output "$tc">>key.txt
            $worksheet.Cells.Item($i, 4).Value2="$uid"



                    #get any reference and add uid to it here
       # $uid=New-Guid

       #look into decrypt with this as a feature
